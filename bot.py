...
OPENAI_KEY = os.environ.get("OPENAI_API_KEY", "").strip()  # для Whisper голос→текст

if not TOKEN:
    print("❌ ОШИБКА: BOT_TOKEN не задан в переменных окружения!")
    sys.exit(1)

# Убрали kill других процессов — на Railway это не нужно и опасно
# Убрали time.sleep(3) — замедлял старт
...
async def flash_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Вариант 1: Whisper через OpenAI API (лучшее качество) ──
        if OPENAI_KEY:
        ...
        # ── Вариант 2: speech_recognition (запасной, если нет OPENAI_KEY) ──
        if not FFMPEG_PATH:
        ...
        try:
            ...
            def recognize():
                ...
                try:
                    ...
                    logger.error(f"SR request error: {e}")
                    return None
            ...
            await status.edit_text(
                ...
                "Добавь `SpeechRecognition` в requirements.txt, или задай `OPENAI_API_KEY` для Whisper.",
                parse_mode=ParseMode.MARKDOWN
            )
