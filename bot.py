...
CITY_MAP = {
    ...
    "кст": "Kostanay", "аст": "Astana", "алм": "Almaty", "нск": "Novosibirsk",
    "крд": "Krasnodar", "чел": "Chelyabinsk", "тмн": "Tyumen"
}

# Настройка ИИ (переход на стабильный метод)
...
async def ask_gemini(prompt, photo_path=None):
    try:
        ...
        return f"❌ Ошибка ИИ: Скорее всего превышен лимит или ключ не активен. {str(e)[:50]}"
...
async def mail(m: types.Message):
    ...
    await m.answer(f"📧 Твоя почта: `{l}@1secmail.com`", parse_mode="Markdown", reply_markup=kb.as_markup())
...
async def chk(c: types.CallbackQuery):
    ...
    await c.message.answer(f"📩 Тело письма:\n{msg['textBody']}")
    await c.answer()
...
async def handle_all(m: types.Message):
    ...
    if t.startswith("флеш вопрос"):
        ...
        city_raw = t.replace("флеш погода", "").strip()
        city = CITY_MAP.get(city_raw, city_raw) or "Kostanay"
        r = requests.get(f"http://api.openweathermap.org/data/2.5/weather?q={city}&appid={WEATHER_KEY}&units=metric&lang=ru").json()
        if r.get("cod") == 200:
            await m.reply(f"🌤 {r['name']}: {r['main']['temp']}°C")
        else: await m.reply("❌ Город не найден.")
...
async def dl_media(m, url, mode):
    ...
    try:
        ...
        await s.edit_text(f"❌ Ошибка: {str(e)[:60]}")
    finally:
        if p and os.path.exists(p): os.remove(p) # Удаляем файл всегда
