...
async def flash_roll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c = "🏆 МАКСИМУМ!" if roll == 100 else "🔥 Отлично!" if roll >= 80 else "😎 Неплохо!" if roll >= 50 else "😅 Так себе..." if roll >= 20 else "💀 Провал!"
...
async def flash_weather(update: Update, context: ContextTypes.DEFAULT_TYPE, city=None):
    if not city: await update.message.reply_text("❗ `флеш погода Алматы`", parse_mode=ParseMode.MARKDOWN); return
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={WEATHER_API_KEY}&units=metric&lang=ru") as r:
                if r.status != 200: await update.message.reply_text(f"❌ Город *{city}* не найден.", parse_mode=ParseMode.MARKDOWN); return
                d = await r.json()
        icons = {"Clear":"☀️","Clouds":"☁️","Rain":"🌧","Snow":"❄️","Thunderstorm":"⛈","Drizzle":"🌦","Mist":"🌫","Fog":"🌫"}
        await update.message.reply_text(f"{icons.get(d['weather'][0]['main'],'🌡')} *{d['name']}*\n🌡 *{d['main']['temp']:.0f}°C* (ощущается {d['main']['feels_like']:.0f}°C)\n💧 {d['main']['humidity']}%\n💨 {d['wind']['speed']} м/с", parse_mode=ParseMode.MARKDOWN)
    except: await update.message.reply_text("❌ Ошибка.")
